...
def signal_text(signal: Signal) -> str:
    ...
    return (
        ...
        f"Stage: {signal.opportunity_stage} | Heat: {signal.opportunity_heat}\n"
        f"Prediction: {signal.prediction_score if signal.prediction_score is not None else '—'}/100 | "
        f"Breakout readiness: {signal.breakout_readiness if signal.breakout_readiness is not None else '—'}/100\n"
        f"Prediction state: {signal.prediction_state} | Direction: {signal.prediction_direction}\n"
        f"False breakout risk: {signal.false_breakout_risk if signal.false_breakout_risk is not None else '—'}/100 | "
        f"Horizon: {signal.prediction_horizon}\n"
        f"Trigger: {signal.trigger_score if signal.trigger_score is not None else '—'}/100 | "
        f"State: {signal.trigger_state} | Type: {signal.trigger_type}\n"
        f"Trigger confirmed: {'YES' if signal.trigger_confirmed else 'NO'}\n"
        f"Entry Optimizer: {signal.entry_optimizer_action} "
        f"({signal.entry_optimizer_score if signal.entry_optimizer_score is not None else '—'}/100)\n"
        f"Оптимальная зона: {signal.entry_optimizer_zone_low:.8g}–{signal.entry_optimizer_zone_high:.8g} | "
        f"цель входа {signal.entry_optimizer_recommended_price:.8g}\n"
        f"Ожидание: {signal.entry_optimizer_wait_atr:.2f} ATR "
        f"({signal.entry_optimizer_wait_percent:.2f}%), TTL {signal.entry_optimizer_ttl_candles} свеч.\n"
...
async def confirm_prepare(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        f"Position Optimizer: {plan.position_optimizer_tier} "
        f"({plan.position_optimizer_score}/100, "
        f"×{plan.position_optimizer_multiplier:.2f})\n"
